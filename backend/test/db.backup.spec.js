...
describe('db backups and read error handling', () => {
  ...
  test('createBackup handles unlink errors gracefully', async () => {
    const { dbFile, backupDir } = getPaths();
    // ensure db exists
    await writeDB({ intermediacoes: [] });
    ...
    // make unlink fail for the first file to delete
    let first = true;
    jest.spyOn(fsp, 'unlink').mockImplementation((p) => {
      if (first) { first = false; return Promise.reject(new Error('mock unlink fail')); }
    ...
    const files = fs.readdirSync(backupDir).filter(f => f.startsWith('db-') && f.endsWith('.json')).sort();
    // if an unlink fails once, we may still have one extra file — ensure function handled the error gracefully
    expect(files.length).toBeLessThanOrEqual(31);

    fsp.unlink.mockRestore();
  });

  test('createBackup writes db if missing', async () => {
    const { dbFile, backupDir } = getPaths();
    ...
    // backup for today should exist
    const todayPattern = new RegExp(`^db-${new Date().toISOString().slice(0,10)}\\.json$`);
    const files = fs.readdirSync(backupDir).filter(f => todayPattern.test(f));
